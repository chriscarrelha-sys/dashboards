...
### Commercial systems — one source of truth for pricing & entitlements
- **DB-backed catalog** (`CommercialPlan`/`PlanPrice`/`PlanEntitlement`, seeded idempotently) is the single authority the public pricing page, checkout, Stripe mappings, and entitlement enforcement all read from — no pricing constant lives in a component. Money is **integer cents**; prices and vendor rates are **effective-dated**.
- **Plans:** Case Essentials $59/$590 · **Litigation Pro $179/$1,790 (Most Popular)** · Litigation Command $399/$3,990. Founding pricing ($49/$149/$349) for the first 100 accounts, cap enforced transactionally, locked while continuously active.
- **Entitlement service** (`lib/commercial/entitlements.ts`): `canCreateCase`/`canRunAIAction`/`canUploadBytes`/`canProcessPages`/`canRunAdvancedWorkflow`/`consumeUsage`/`reverseUsage`/`getUsageSummary` — every module goes through it. Usage is a **transaction-safe, idempotency-keyed append-only ledger** (no double-charge, no negative balance; included allowance before rollover purchased actions).
- **AI Case Actions** — a customer-facing weighted unit; tokens/model prices/API keys are **never** exposed. The app shows the estimate before running and never over-charges it.
- **Internal cost accounting** — every AI request records tokens + calculated cost from effective-dated vendor rates (`AIProviderCostRecord`); `/admin/commercial` shows MRR/ARR, plan mix, founding seats, and **per-plan contribution margin** (estimated fields labeled). Provider cost is never shown to customers.
- **Trials, add-ons, upgrades/downgrades, cancellation** with a 30-day export grace; **Stripe** mapping + idempotent webhooks (mock until `STRIPE_SECRET_KEY`+`STRIPE_LIVE`). Surfaces: `/pricing`, `/settings/plan`, `/admin/commercial`. See [`docs/PRICING.md`](docs/PRICING.md).

See [`docs/`](docs/) for [architecture](docs/ARCHITECTURE.md), [data model](docs/DATA-MODEL.md), the [verification workflow](docs/VERIFICATION-WORKFLOW.md), [filing/service/research workflows](docs/FILING-WORKFLOW.md), [security threat model](docs/SECURITY.md), [deployment](docs/DEPLOYMENT.md), [launch plan](docs/LAUNCH.md), [production blockers](docs/PRODUCTION-BLOCKERS.md), [pricing & commercial systems](docs/PRICING.md), [operating manual](docs/OPERATING-MANUAL.md), [admin runbook](docs/ADMIN-RUNBOOK.md), [integrations matrix](docs/INTEGRATIONS-MATRIX.md), [companion spec](docs/COMPANION.md), [operations runbook & DR](docs/RUNBOOK.md), [privacy](docs/PRIVACY.md), roadmap, and limitations.
