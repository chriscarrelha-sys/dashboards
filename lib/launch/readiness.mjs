...
export async function runLaunchReadiness(ctx = {}) {
  ...
  if (env.STORAGE_PROVIDER === 's3') {
    ...
    checks.push(s3Ready
      ? result('storage-connectivity', 'Private object storage', 'pass', `S3-compatible bucket ${env.S3_BUCKET}.`)
      : result('storage-connectivity', 'Private object storage', 'failure', 'STORAGE_PROVIDER=s3 but S3_ENDPOINT/S3_BUCKET/credentials are incomplete.'));
    checks.push(result('private-object-enforcement', 'Objects are private', s3Ready ? 'pass' : 'warning', 'Uploads are served only through the authorized in-app download route.'));
    checks.push(result('signed-url-behavior', 'Signed URLs expire', 'warning', 'Downloads are proxied + auth-gated; direct time-limited signed URLs are a later enhancement.'));
