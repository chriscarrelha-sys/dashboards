...
**Reconcile the court's calendar; do not dismiss it.** PACER's Deadlines/Hearings report
is generated by the rules the clerk applies. A disagreement between it and an independent
computation means the computation is **incomplete** — find the model that explains the
court's date before doing anything else.

Note in particular that a **Submission Deadline** is not a response deadline: it is the
date the motion goes to the judge, normally one business day after the response period
closes. Decoding it across several motions reveals both the response period the clerk is
applying and whether the Rule 6(d) mail addition is in effect — usually the largest
unverified assumption on any deadline sheet.

Only where no model explains the discrepancy does the system record both dates, flag the
conflict, and operate to the earliest candidate. Reaching for that fallback first produces
deadlines that are wrong in the safe direction, which still misleads and costs real
preparation days.
